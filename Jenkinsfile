      ...
      steps {
        withCredentials([string(credentialsId: 'final', variable: 'SONAR_TOKEN')]) {
          withSonarQubeEnv('sonar') {
            sh '''#!/usr/bin/env bash
set -euo pipefail
SCANNER_HOME="$(tool 'sonar-scanner')"   # Resolve the Tool here
export PATH="$SCANNER_HOME/bin:$PATH"

echo "[INFO] Using sonar-scanner from: $SCANNER_HOME"
sonar-scanner \
    ...
    stage('Quality Gate') {
      steps {
        timeout(time: 3, unit: 'MINUTES') {
          script {
            def qg = waitForQualityGate()   // no credentialsId needed
